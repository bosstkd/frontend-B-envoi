...
infoForm() {
  ...
  AddData(bonIndex,Id){
    ...
    this.dialog.open(AddLdepotComponent, dialogConfig).afterClosed().subscribe(b10=>{
      ...
      this.calcul( this.calService.poid, this.calService.destination);
    });
  }
  ...
  async calcul(poid:number, destination:string){
    let total = 0;
    this.calService.calcul(poid,destination).then(async data=>{
      total = await data as number;
